...
const { latLngToCell } = require('../server/node_modules/h3-js');
...
const evidence = {
  spawnId: resource.id,
  playerA: { status: gatherA.status, body: gatherA.body },
  playerB: { status: gatherB.status, body: gatherB.body },
};
writeFileSync(scratch, JSON.stringify(evidence, null, 2));
server.close();
db.close();
